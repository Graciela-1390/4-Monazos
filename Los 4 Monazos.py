...
#          realiza un cálculo matemático para generar el código secreto, y ls guarda en un archivo .txt.
# -------------------------------------------------
def generar_codigo_secreto(nombre_archivo):
    ...
    while True:
        ...
        # Valida que haya exactamente 10 elementos.
        if len(lista_secuencia) != 10:
            print("\nError: Debe ingresar exactamente 10 números.")
            continue 

        # Valida que todos sean dígitos y estén entre 0 y 9.
        todos_validos = True
        ...
        # Tomamos los primeros 4 números de la lista mezclada.
        combinacion = lista_digitos[:4]
...
# Cada intento se guardará como [combinacion, aciertos, coincidencias].
# Devuelve: lista vacía que representará el tablero de juego.
# -------------------------------------------------
def crear_tablero():
    tablero = []
# Prepara 10 filas vacías (van a llenarse conforme avanza el juego).
    for i in range(10):
...
def imprimir_tablero(tablero):
    ...
    for fila in tablero:
        if fila[0] is not None: # Solo muestra los intentos ya jugados.
            # Convierte la combinación a cadena (str), sin comas.
            combinacion_cadena = "".join(str(digito) for digito in fila[0]) # Convierte la lista de enteros a cadena.
            print(f"{numero_intento}  {combinacion_cadena}     | {fila[1]} | {fila[2]}")
            numero_intento += 1
...
def calculo_resultado(enteros_combinacion, combinacion):
    ...
    for i in range(len(intento)):
        ...
        try: # Buscamos la primera aparición de 'valor' en 'secreto' (si existe).
            ...
            #  No está en 'secreto' (o ya fue consumido), no hacemos nada.
            pass
